-- Created by Ebean DDL
-- To stop Ebean DDL generation, remove this comment (both lines) and start using Evolutions

-- !Ups

-- apply changes
create table company (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  name                          varchar(255),
  constraint pk_company primary key (id)
);

create table crop (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  name                          varchar(255),
  introduced                    timestamp,
  discontinued                  timestamp,
  company_id                    bigint,
  user_id                       bigint,
  constraint pk_crop primary key (id)
);

create table field (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  name                          varchar(255),
  north_east_latitude           double,
  north_east_longitude          double,
  south_west_latitude           double,
  south_west_longitude          double,
  user_id                       bigint,
  constraint pk_field primary key (id)
);

create table pesticide_registration (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  registration_number           varchar(255),
  usage                         varchar(255),
  pesticide_type                varchar(255),
  pesticide_name                varchar(255),
  abbreviation                  varchar(255),
  crop_name                     varchar(255),
  application_location          varchar(255),
  target_pest_disease           varchar(255),
  purpose                       varchar(255),
  dilution_amount               varchar(255),
  spray_volume                  varchar(255),
  usage_time                    varchar(255),
  main_agent_usage_count        varchar(255),
  usage_method                  varchar(255),
  fumigation_time               varchar(255),
  fumigation_temperature        varchar(255),
  applicable_soil               varchar(255),
  applicable_zone_name          varchar(255),
  applicable_pesticide_name     varchar(255),
  mixture_count                 varchar(255),
  active_ingredient_1_total_usage varchar(255),
  active_ingredient_2_total_usage varchar(255),
  active_ingredient_3_total_usage varchar(255),
  active_ingredient_4_total_usage varchar(255),
  active_ingredient_5_total_usage varchar(255),
  constraint pk_pesticide_registration primary key (id)
);

create table soil_diagnostic (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  diagnostic_date               date,
  field_id                      bigint,
  user_id                       bigint,
  cec                           decimal(16,3),
  ec                            decimal(16,3),
  ph_h2_o                       decimal(16,3),
  ph_kcl                        decimal(16,3),
  nh4_n                         decimal(16,3),
  k2_o                          decimal(16,3),
  phosphorus_absorption_coefficient decimal(16,3),
  available_nitrogen            decimal(16,3),
  p2_o5                         decimal(16,3),
  ca_o                          decimal(16,3),
  no3_n                         decimal(16,3),
  humus                         decimal(16,3),
  mg_o                          decimal(16,3),
  constraint pk_soil_diagnostic primary key (id)
);

create table app_user (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  username                      varchar(255),
  email                         varchar(255),
  password                      varchar(255),
  reset_token                   varchar(255),
  reset_token_expires           timestamp,
  is_admin                      boolean,
  constraint pk_app_user primary key (id)
);

create table work_history (
  id                            bigint generated by default as identity not null,
  created_at                    timestamp,
  updated_at                    timestamp,
  date                          date,
  start_time                    time,
  end_time                      time,
  field_id                      bigint,
  crop_id                       bigint,
  content                       varchar(255),
  user_id                       bigint,
  constraint pk_work_history primary key (id)
);

-- foreign keys and indices
create index ix_crop_company_id on crop (company_id);
alter table crop add constraint fk_crop_company_id foreign key (company_id) references company (id) on delete restrict on update restrict;

create index ix_crop_user_id on crop (user_id);
alter table crop add constraint fk_crop_user_id foreign key (user_id) references app_user (id) on delete restrict on update restrict;

create index ix_field_user_id on field (user_id);
alter table field add constraint fk_field_user_id foreign key (user_id) references app_user (id) on delete restrict on update restrict;

create index ix_soil_diagnostic_field_id on soil_diagnostic (field_id);
alter table soil_diagnostic add constraint fk_soil_diagnostic_field_id foreign key (field_id) references field (id) on delete restrict on update restrict;

create index ix_soil_diagnostic_user_id on soil_diagnostic (user_id);
alter table soil_diagnostic add constraint fk_soil_diagnostic_user_id foreign key (user_id) references app_user (id) on delete restrict on update restrict;

create index ix_work_history_field_id on work_history (field_id);
alter table work_history add constraint fk_work_history_field_id foreign key (field_id) references field (id) on delete restrict on update restrict;

create index ix_work_history_crop_id on work_history (crop_id);
alter table work_history add constraint fk_work_history_crop_id foreign key (crop_id) references crop (id) on delete restrict on update restrict;

create index ix_work_history_user_id on work_history (user_id);
alter table work_history add constraint fk_work_history_user_id foreign key (user_id) references app_user (id) on delete restrict on update restrict;


-- !Downs

-- drop all foreign keys
alter table crop drop constraint if exists fk_crop_company_id;
drop index if exists ix_crop_company_id;

alter table crop drop constraint if exists fk_crop_user_id;
drop index if exists ix_crop_user_id;

alter table field drop constraint if exists fk_field_user_id;
drop index if exists ix_field_user_id;

alter table soil_diagnostic drop constraint if exists fk_soil_diagnostic_field_id;
drop index if exists ix_soil_diagnostic_field_id;

alter table soil_diagnostic drop constraint if exists fk_soil_diagnostic_user_id;
drop index if exists ix_soil_diagnostic_user_id;

alter table work_history drop constraint if exists fk_work_history_field_id;
drop index if exists ix_work_history_field_id;

alter table work_history drop constraint if exists fk_work_history_crop_id;
drop index if exists ix_work_history_crop_id;

alter table work_history drop constraint if exists fk_work_history_user_id;
drop index if exists ix_work_history_user_id;

-- drop all
drop table if exists company;

drop table if exists crop;

drop table if exists field;

drop table if exists pesticide_registration;

drop table if exists soil_diagnostic;

drop table if exists app_user;

drop table if exists work_history;

